			...
			function library:New(name) 
				...
				do
					...
					TextLabel.Text = "Floppaware - Private | User: dev | Version 1.1 [dev] [beta]"
					...
					local function SSYFZC_fake_script() -- TextLabel.LocalScript 
						...
						game:GetService("RunService").Heartbeat:Connect(function()
							text.Text = "Floppaware - Private | User: dev | Version 1.1 [dev] [beta]"
						end)
    ...
    setreadonly(mt, true)
        ...
        --]]
			...
			chat:Element('Toggle', 'random killsay', nil, function(tbl)
				if tbl.Toggle then
					...
					for i = 1, 9e9 do
						...
						local chatmessages = {
							"No Floppaware? L",
							"Seems like u died to Floppaware",
							"You just got 1'ed by Floppaware",
							"Died to Floppaware? Just imagine",
							"1",
							"Floppaware UID!?, right none!!",
							"1, Couldn't have done it without Floppaware",
							"Died to Floppaware",
							...
							"RAWR SIT NN DOG!",
							"What cheat is that? bet its not Floppaware",
							"Wait arent you the guy who's mom I fucked last night?",	
							"It looks like your face is on 'Killed By Nixus' list.",
							"Don't you love Floppaware, despite how many times it tapped you?",
							"1'ed by Floppaware, LOL!",
							"Dang imagine dying to Floppaware.",
							"HAH GOTTEM!",
							"Floppaware hijacked your mama's card",
							"Do you not smile?, i mean i feel ya. You got tapped by Floppaware no wonder ur not smiling.",
							"Floppaware killed your cheat",
							"Floppaware roasting yo mama!",
							"yo what happened! did your cheat miss on Floppaware!?",
						}
			...
			grenades:Element("Button", "crash server", {}, function() 
				local A_1 = "🖥️🖥️ Server smoked by Floppaware Get Floppaware now. 🖥️🖥️"
				local A_2 = false
				local A_3 = "Innocent" 
				local A_4 = false
				local A_5 = true
			...
			addons:Element('TextBox', 'watermark text', {placeholder = 'text here', default = {text = 'Nixus'}}, function(tbl)
			...
			LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current) 
				...
				if values.misc.chat["kill say"].Toggle then 
					game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "Floppaware", false, "Innocent", false, true) 
				end 
			end) 
