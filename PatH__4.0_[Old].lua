...
warn("Dont search god i love trampolining in discord gifs :troll:")
getgenv().OwO = true
...
TextLabel.Text = "PastaHook__4.0 | Vc.s'"
...
Tabs.Position = UDim2.new(0.033, 0,0.087, 0) 
Tabs.Size = UDim2.new(0, 590,0, 514)				
...
TabGui.Size = UDim2.new(0, 590,0, 514) 
TabGui.Position = UDim2.new(0, -8, 0, 3) 
	...
	function MSector:Tab(text) 
		...
		function tab:Element(type, text, data, callback) 
			...
			if type == "Jumbobox" then 
				...
				Drop.ZIndex = 5 
				Drop.ScrollBarImageColor3 = Color3.fromRGB(172, 208, 255) 
					...
					do 
						...
						Button.MouseButton1Down:Connect(function() 
							if table.find(Element.value.Jumbobox, v) then 
								...
								library:Tween(TextLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = Color3.fromRGB(172, 208, 255)}) 
							end 
							updatetext() 
			...
			elseif type == "ToggleKeybind" then 
				...
				local function update() 
					if Element.value.Toggle then
						...
						tween = library:Tween(Color, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = Color3.fromRGB(172, 208, 255)}) 
						library:Tween(TextLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = Color3.fromRGB(255, 255, 255)}) 
					else
			...
			elseif type == "Button" then 
				...
				Button_2.MouseButton1Down:Connect(function() 
					TextLabel.TextColor3 = Color3.fromRGB(172, 208, 255) 
		...
		if type == "ScrollDrop" then 
			...
			for i,v in ipairs(joe) do 
				do 
					...
					Frame.ScrollBarImageColor3 = Color3.fromRGB(172, 208, 255) 
						...
						for _,bad in pairs(v.Frame:GetChildren()) do 
							if bad:IsA("TextButton") then 
								...
								if bad.Name == Element.value.Scroll[v.Name] then 
									bad.TextLabel.TextColor3 = Color3.fromRGB(172, 208, 255) 
								end 
							end 
						end 
					end 
		...
		elseif type == "Jumbobox" then 
			...
			function Element:SetValue(val) 
				...
				for i,v in pairs(Drop:GetChildren()) do 
					if v.Name ~= "UIListLayout" then 
						if table.find(val.Jumbobox, v.Name) then 
							v.TextLabel.TextColor3 = Color3.fromRGB(172, 208, 255) 
						else 
							v.TextLabel.TextColor3 = Color3.fromRGB(200, 200, 200) 
						end 
					end 
...
local luaname = string.gsub(lua, "floppalua\\", "") 
table.insert(allluas, luaname) 
end 

RunService.RenderStepped:Wait() 

local gui = library:New("PastaHook__4.0") 
...
luascripts:Element("Scroll", "lua", {options = allluas, Amount = 7}) 
luascripts:Element("Button", "load", {}, function() 
loadstring(readfile("floppalua\\"..values.luas["lua scripts"].lua.Scroll))() 
end) 


local server = luas:Sector("Server Environment", "Right") 
...
server:Element("Button", "fix stuck on voting", nil, function()
...
server:Element("Button", "server crasher", nil, function()      
while true do
pcall(function()
game:GetService("RunService").RenderStepped:Wait()
for i = 1,100,1 do
...
local god = luas:Sector("Godmode Methods", "Left")
god:Element("Button", "Hostage", nil, function()
print("enabled")
local ReplicatedStorage = game:GetService("ReplicatedStorage");
local ApplyGun = ReplicatedStorage.Events.ApplyGun;
ApplyGun:FireServer({Model = ReplicatedStorage.Hostage.Hostage, Name = "USP"}, game.Players.LocalPlayer);
end)
god:Element("Button", "Remove Animations", nil, function()
...
god:Element("Button", "Fall Damage", nil, function() pcall(function()
...
local plr = luas:Sector("Player Exploits", "Left")
plr:Element("Button", "DDoS Hitboxes", nil, function()
game.Players.localPlayer.Character.UpperTorso.Waist:Destroy()
end)

plr:Element("Button", "invisible", nil, function()
local player = game.Players.LocalPlayer																																																									    --neverblox.cc
local parent1 = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
player.Character.HumanoidRootPart.CFrame = CFrame.new()
local parent2 = game.Players.LocalPlayer.Character.LowerTorso.Root:Clone()
player.Character.LowerTorso.Root:Destroy()
parent2.Parent = game.Players.LocalPlayer.Character.LowerTorso
wait()
player.Character.HumanoidRootPart.CFrame = parent1
end)

plr:Element("Button", "force defuse", nil, function()
if game.Players.LocalPlayer.Character and workspace.Map.Gamemode.Value == "defusal" and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and workspace:FindFirstChild("C4") then
game.Players.LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0, 0, 0)
local cT = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
...
game.Players.LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0, 0, 0)
wait(0.25)
if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and workspace:FindFirstChild("C4") and workspace.C4:FindFirstChild("Defusing") and workspace.C4.Defusing.Value == game.Players.LocalPlayer then
game.Players.LocalPlayer.Backpack.Defuse:FireServer(workspace.C4)
end
game.Players.LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0, 0, 0)
wait(0.2)
game.Players.LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0, 0, 0)
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = cT
game.Players.LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0, 0, 0)
...
aimbot:Element("Dropdown", "alternative prediction", {options = {"off", "manual", "automatic", "adaptive"}})
aimbot:Element("Slider", "prediction multiplier", {min = 1, max = 100, default = 20})
...
aimbot:Element("Dropdown", "knifebot speed", {options = {"normal", "rapid"}}) 
aimbot:Element("Dropdown", "knifebot type", {options = {"normal", "zesty ahh knifebot", "gay tf"}})
...
antiaim:Element("Slider", "extend value", {min = 0, max = 20, default = 2}) 
...
others:Element("Dropdown", "leg movement", {options = {"off", "slide", "slidewalk"}}) 
...
fakelag:Element('Slider', 'set ping', {min = -100, max = 1800, default = 0})
...
game:GetService('ReplicatedStorage').Events.UpdatePing:FireServer( values.rage.fakelag['set ping'].Slider/10)
            ...
            fakelag:Element("Dropdown", "visualize lag type", {options = {"Hitbox", "Ball"}})
							...
							if values.rage.fakelag.amount.Dropdown == 'static' then 
								...
								if values.rage.fakelag['visualize lag'].Toggle then
									for _,hitbox in pairs(LocalPlayer.Character:GetChildren()) do
										...
										if values.rage.fakelag["visualize lag type"].Dropdown == "Hitbox" then
										...
										elseif values.rage.fakelag["visualize lag type"].Dropdown == "Ball" then
											local Part = Instance.new("Part")
			...
			freezebusy1 = false
			freezebusy2 = false
fakelag:Element('ToggleKeybind', 'freezeclip (gyro)', nil, function(tbl)
...
freezebusy1 = false
freezebusy2 = false
fakelag:Element("ToggleKeybind", "freezeclip (cframe)", {}, function(tbl)
if tbl.Toggle and tbl.Active then
...
local rageworld = rage:Sector("World", "Right")
rageworld:Element('ToggleKeybind', 'around the world')
rageworld:Element('Slider', 'height', {min = -200, max = 200, default = 0})
rageworld:Element('Slider', 'distance', {min = 1, max = 500, default = 0})
rageworld:Element('Slider', 'speed', {min = 1, max = 100, default = 0})
...
exploits:Element("Slider", "hps", {min = 1, max = 255, default = 1}) 
...
if values.rage.exploits["quick peek"].Toggle and values.rage.exploits["quick peek"].Active and LocalPlayer.Character then
...
local step1
	...
	--values.rage.luascripts.Player.Dropdown
				if Players[values.rage["exploits"].Player.Dropdown].Character and Players[values.rage["exploits"].Player.Dropdown].Team ~= LocalPlayer.Team and Players[values.rage["exploits"].Player.Dropdown].Character:FindFirstChild("UpperTorso") then
					...
					local oh2 = Players[values.rage["exploits"].Player.Dropdown].Character.Head.CFrame.p
...
sethiddenproperty(game.Lighting, "Technology", values.visuals.effects["shadowmap technology"].Toggle and "ShadowMap" or "Legacy") 
end) 
local mS = game.Lighting
effects:Element("ToggleTrans", "fog effects (buggy)", {default = {Color = Color3.fromRGB(255,255,255)}}, function(val)
while wait() do
	if val.Toggle then
			mS.FogEnd = 150
			mS.FogStart = 50
			...
			mS.FogEnd = 9999999999
			mS.FogStart = 99999999999
		end
	end
end)
...
world:Element("Toggle", "hitsounds on warmup") 
...
world:Element("Slider", "sound volume", {min = 1, max = 12, default = 6}) 
...
movement:Element("Dropdown", "type", {options = {"velocity", "gyro", "cframe", "cframev2"}}) 
movement:Element("Dropdown", "gyro velocity", {options = {"extended", "normal"}})
...
local chair = {"Want to get better? Are you tired of standing while gaming? We represent you the one and only 0.99$ chair, Hurry up! This deal will expire by the next two days.", "What's here? What's theres? bro It's just a damn chair.", "Today, We represent you the cheapest and the only foldable chair in the world! Costing you 5$ and is sold separately.", "Dear santa, What am I getting for christmas? Chairs!", "Fuck orphans, This chair is the best one that we made yet, Costing you only 12.99$ It also comes with dildos! Buy now!", "Man I'm done playing this game my legs are tired! Today, We represent you the one and only pewdiepie gaming chair! Only costing you 50$ Hurry up! (this job sucks ass jeez)", "Do you love cats? Today we have created a chair with cats! only 1.99$ today! buy now in petsovernight.com !!"}
...
if values.misc.chat.type.Dropdown == "Gaming Chair" then
wait(values.misc.chat["speed (ms)"].Slider/1000)  
local A_1 = chair[math.random(1,table.getn(chair))]
local A_2 = false
local A_3 = "Innocent"
local A_4 = false
local A_5 = false
...
chat:Element("Dropdown", "type", {options = {"Effortfully", "Amen", "Gaming Chair"}})
chat:Element('Slider', 'speed (ms)', {min = 150, max = 500, default = 250})
chat:Element('Toggle', 'amen')
chat:Element("Toggle", "kill say") 
chat:Element("Toggle", "NNs dont talk")
chat:Element("Dropdown", "Talk type", {options = {"Default", "OwO"}})
...
library:SetSpectatorVisible(tbl.Toggle) 
	...
	TextLabel.Font = Enum.Font.Ubuntu
	TextLabel.Text = "  PastaHook__4.0  |  [Main Build]  |  counter-blox   |  fps 000  |  000ms  "
	TextLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
	TextLabel.TextSize = 11.000
	TextLabel.TextStrokeTransparency = 0.000
	...
	local function FHUZWEW_fake_script() -- TextLabel.LocalScript 
		...
		game:GetService('RunService').RenderStepped:Connect(function(step)
			if (tick()-lasttick)*1000 > 25 then
				lasttick = tick()				
				script.Parent.Text = "   " ..valuewtr.. "  |  Vc.s', [Main Build]  |  " ..GetName.Name.. "  |  " ..math.floor(1/step).. " fps | "..math.floor(game:GetService('Stats').Network.ServerStatsItem["Data Ping"]:GetValue()).. "ms   "
			end
		end)
	end
...
addons:Element('ToggleColor', 'Menu Accent', {default = {Toggle = true, Color = Color3.fromRGB(172, 208, 255)}}, function(tbl)
if tbl.Toggle then
...
CreateHitElement(" Welcome "..LocalPlayer.Name.." to Pastahook!  ",Color3.new(1,1,1),3, 0, 280, 0, 22)
wait(0.5)
CreateHitElement(" Press table.insert to open and close the menu!  ",Color3.new(1,1,1),3, 0, 245, 0, 22)
		...
		if b3.Team ~= b3.Parent.LocalPlayer.Team then
			if b3.Character and b3.Character:FindFirstChild("UpperTorso") and b3.Parent.LocalPlayer.Character and b3.Parent.LocalPlayer.Character:FindFirstChild("EquippedTool") then
				if b3.Character:FindFirstChild("Humanoid") and b3.Character.Humanoid.Health > 0 then
					...
					if values.rage.exploits["hps"].Slider > 1 then
						for i=2, values.rage.exploits["hps"].Slider do
...
if table.find(values.visuals.effects.removals.Jumbobox, "scope lines") then 
	...
	NewScope.Enabled = false 
					...
					if UserInputService:IsKeyDown("Space") and values.misc.movement["bunny hop"].Toggle then 
						...
						if values.misc.movement.type.Dropdown == "gyro" and values.misc.movement["gyro velocity"].Dropdown == "extended" then
						...
						if add == 0 and values.misc.movement.direction.Dropdown == "directional" and not UserInputService:IsKeyDown("W") then 
							...
							elseif values.misc.movement.type.Dropdown == 'velocity' then
								BodyVelocity:Destroy()
								Root.Velocity = Vector3.new(rot.LookVector.X * (values.misc.movement["speed"].Slider * 2), Root.Velocity.y, rot.LookVector.Z * (values.misc.movement["speed"].Slider * 2))
							elseif values.misc.movement.type.Dropdown == "cframev2" then
								BodyVelocity:Destroy()
...
if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") and Player.Team ~= "TTT" and (Player.Team ~= LocalPlayer.Team or values.visuals.players.teammates.Toggle) and Player.Character:FindFirstChild("Gun") and Player.Character:FindFirstChild("Humanoid") and Player ~= LocalPlayer then 
 ...
 Player.Character.UpperTorso.Waist.C0 = CFrame.new(Vector3.new(0, 0.6, 0)) * CFrame.Angles(0, 0, 0)  
                    ...
                    if values.rage.aimbot["forward tracking"].Dropdown == "forward" then 
											...
											if ( b.x ~= a.x ) and ( b.z ~= a.z ) then
												if ( b.x > a.x ) and ( b.z > a.z ) then 
													Player.Character.Head.Neck.C0 = CFrame.new((values.rage.aimbot["retardation amount"].Slider), 0, (values.rage.aimbot["retardation amount"].Slider))
													Player.Character.UpperTorso.Waist.C0 = CFrame.new((values.rage.aimbot["retardation amount"].Slider), 0, (values.rage.aimbot["retardation amount"].Slider))
											...
											elseif (  b.x ~= a.x ) then
												if ( b.x > a.x ) then 
													Player.Character.UpperTorso.Waist.C0 = CFrame.new((values.rage.aimbot["retardation amount"].Slider), 0, 0)
													Player.Character.Head.Neck.C0 = CFrame.new((values.rage.aimbot["retardation amount"].Slider), 0, 0)
												else
													Player.Character.UpperTorso.Waist.C0 = CFrame.new((values.rage.aimbot["retardation amount"].Slider), 0, 0)
													Player.Character.Head.Neck.C0 = CFrame.new((values.rage.aimbot["retardation amount"].Slider), 0, 0)
												end
												...
												if ( b.z > a.z ) then 
													Player.Character.Head.Neck.C0 = CFrame.new(0, 0, (values.rage.aimbot["retardation amount"].Slider))
													Player.Character.UpperTorso.Waist.C0 = CFrame.new(0, 0, (values.rage.aimbot["retardation amount"].Slider))
												else
													Player.Character.Head.Neck.C0 = CFrame.new(0, 0, (values.rage.aimbot["retardation amount"].Slider))
													Player.Character.UpperTorso.Waist.C0 = CFrame.new(0, 0, (values.rage.aimbot["retardation amount"].Slider))
												end
										...
										if values.rage.aimbot["forward tracking"].Dropdown == "predictive" then      
											local a = Player.Character.HumanoidRootPart.CFrame
											wait()
											local b = Player.Character.HumanoidRootPart.CFrame
                      					...
                      					if values.rage.aimbot["forward tracking"].Dropdown == "reversed" then 
											local a = Player.Character.HumanoidRootPart.CFrame
											wait()
											local b = Player.Character.HumanoidRootPart.CFrame
...
if workspace:FindFirstChild("Map") and Client.gun ~= nil and Client.gun ~= "none" and Client.gun.Name ~= "C4" then
	if values.rage.aimbot.enabled.Toggle then 
		local Origin 
		if not values.rage.aimbot["enchance player origin"].Toggle then
		...
		elseif values.rage.aimbot["enchance player origin"].Toggle then
			...
			elseif values.rage.aimbot["origin modifier type"].Dropdown == "extended" then
				Origin = values.rage.aimbot.origin.Dropdown == "character" and LocalPlayer.Character.LowerTorso.Position + Vector3.new(3.2, 4.0, 3.2) or CamCFrame.p
			end
		end
			...
			if Player.Character and Player.Character:FindFirstChild("Humanoid") and not Client.DISABLED and Player.Character:FindFirstChild("Humanoid").Health > 0 and Player.Team ~= "TTT" and not Player.Character:FindFirstChildOfClass("ForceField") and GetDeg(CamCFrame, Player.Character.Head.Position) <= Stats["max fov"].Slider and Player ~= LocalPlayer then 
					if Player.Team ~= LocalPlayer.Team or values.rage.aimbot.teammates.Toggle and Player:FindFirstChild("Status") and Player.Status.Team.Value ~= LocalPlayer.Status.Team.Value and Player.Status.Alive.Value then 
						if Client.gun:FindFirstChild("Melee") and values.rage.aimbot["knifebot"].Toggle then
							...
							if values.rage.aimbot["knifebot speed"].Dropdown == "rapid" then
								Client.DISABLED = false
							end
							...
							if Hit and Hit.Parent == Player.Character then                                    
								...
								if values.rage.aimbot["knifebot type"].Dropdown == "normal" then
									local Arguments = {
										[1] = Hit,
										[2] = Hit.Position,
								...
								elseif values.rage.aimbot["knifebot type"].Dropdown == "zesty ahh knifebot" then
									local Arguments = {
										[1] = Hit.Parent.Head,
										[2] = Hit.Position,
										[3] = LocalPlayer.Character.EquippedTool.Value,
								...
								elseif values.rage.aimbot["knifebot type"].Dropdown == "gay tf" then
									local Arguments = { 
										[1] = Hit, 
										[2] = Hit.Position, 
										[3] = "Banana",
								...
								if EndHit ~= nil and Multipliers[EndHit.Name] ~= nil and #Hits <= 4 then 
									if #Hits == 0 then 
										...
										if Damage >= Stats["minimum damage"].Slider then 
											...
											local wallbang = true
											local penetration 
											if not values.rage.aimbot["old ray cast"].Toggle then
												penetration = Client.gun.Penetration.Value * (0.01 * values.rage.aimbot["penetration"].Slider)	
											elseif values.rage.aimbot["old ray cast"].Toggle then
												penetration = Client.gun.Penetration.Value * (0.02 * values.rage.aimbot["penetration"].Slider)	
											end			
											local limit = 0 
											local dmgmodifier = 1 
											for i = 1, #Hits do 
									...
									if Hit and Multipliers[Hit.Name] ~= nil then 
										...
										if Damage >= Stats["minimum damage"].Slider then 
											...
											if values.rage.aimbot["automatic fire"].Dropdown == "standard" then 
												Client.firebullet() 
												if values.rage.exploits["custom tap"].Toggle and values.rage.exploits["custom tap"].Active then 
													for i = 2, values.rage.exploits["tap amount"].Slider do
													Client.firebullet() 
												end 
												if values.rage.aimbot["ragebot logs"].Toggle then
														CreateHitElement(" Hit "..EndHit.Parent.Name.." in the "..EndHit.Name.."  ",Color3.new(1,1,1),1 * values.rage.aimbot["log time"].Slider, 0, 280, 0, 22)
...
local speed = values.visuals.self["silent angle speed"].Slider/10
local last = tick()
...
if values.rage.others["leg movement"].Dropdown == "slidewalk" then 
	if string.find(args[1].Name, "Run") or string.find(args[1].Name, "Walk") or string.find(args[1].Name, "Jump") then
		args[1] = FakeAnim
	end
end 
...
if values.rage.aimbot["alternative prediction"].Dropdown ~= "off" and RageTarget ~= nil then
	coroutine.wrap(function()
		if Players:GetPlayerFromCharacter(args[1].Parent) or args[1] == RageTarget then
		local tv = Vector3.new(0, 0, 0)
			if values.rage.aimbot["alternative prediction"].Dropdown == "automatic" then
				...
				tv = vel.Unit * vel.Magnitude * vf / 1000 * (vf > 200 and 1.5 or 2)
			elseif values.rage.aimbot["alternative prediction"].Dropdown == "adaptive" then
				local hrp = RageTarget.Parent.HumanoidRootPart.Position 
				local oldHrp = RageTarget.Parent.HumanoidRootPart.OldPosition.Value
				...
				local vh = vg / 1000 * 2 * vel.Magnitude
				local vi = vg / 1000 * 2 * (vel.Magnitude - vh)
...
local haramwords = {"OwO", "owo", "UwU", "uwu", "Ah~", "Oh~", ">-<", ">w<", ">~<"}
...
getsenv(game.Players.LocalPlayer.PlayerGui.GUI.Main.Chats.DisplayChat).createNewMessage = function(plr, msg, teamcolor, msgcolor, offset, line)
	if values.misc.chat["NNs dont talk"].Toggle and plr ~= game.Players.LocalPlayer.Name then
			if values.misc.chat["Talk type"].Dropdown == "Default" then
				msg = "I am retarded."
			else 
				msg = haramwords[math.random(1,table.getn(haramwords))]
			end      
		end
